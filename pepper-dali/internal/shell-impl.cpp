...
void Shell::Configure( int width, int height, ConfigureCallback callback, void* data )
{
  struct wl_display* display;
...
void Shell::GetSurface( wl_client* client, unsigned int id, wl_resource* surfaceResource )
{
  ...
  DALI_LOG_INFO( gPepperShellLogging, Debug::Verbose, "Shell::GetSurface: success. surface = %p\n", surface );
}
...
std::string Shell::GetTitle() const
{
  return mTitle;
}

void Shell::SetAppId( const std::string appId )
{
  mAppId = appId;
}

std::string Shell::GetAppId() const
{
  return mAppId;
}
...
void Shell::OnSurfaceDestroy( pepper_event_listener_t* listener, pepper_object_t* surface, uint32_t id, void* info, void* data )
{
  ...
  DALI_LOG_INFO( gPepperShellLogging, Debug::Verbose, "Shell::OnSurfaceDestroy: success. surface = %p\n", surface );
}
