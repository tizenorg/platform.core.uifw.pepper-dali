...
Integration::Log::Filter* gPepperShellLogging  = Integration::Log::Filter::New( Debug::Verbose, true, "LOG_PEPPER_SHELL" );
#endif
...
static void SurfaceDestroyListener( pepper_event_listener_t* listener, pepper_object_t* surface, uint32_t id, void* info, void* data )
{
  Shell* shell = static_cast< Shell* >( data );

  shell->OnSurfaceDestroy( surface );
}

static void
SurfaceCommitListener( pepper_event_listener_t* listener, pepper_object_t* surface, uint32_t id, void* info, void* data )
{
  Shell* shell = static_cast< Shell* >( data );

  shell->OnSurfaceCommit();
}
...
static void XdgShellBind( struct wl_client* client, void* data, uint32_t version, uint32_t id )
{
  Shell* shell = static_cast< Shell* >( data );

  shell->CreateClient( client, version, id );
}
...
void Shell::Initialize( Pepper::Compositor& compositor )
{
  ...
  if( !wl_global_create( display, &xdg_shell_interface, 1, this, XdgShellBind ) )
  {
...
void Shell::CreateClient( wl_client* client, unsigned int version, unsigned int id )
{
...
void Shell::GetSurface( wl_client* client, unsigned int id, wl_resource* surfaceResource )
{
  ...
  wl_resource_set_implementation( mSurface, &xdgSurfaceImplementation, this, SurfaceResourceDestroy );

  mSurfaceDestroyListener = pepper_object_add_event_listener( reinterpret_cast< pepper_object_t* >( surface ), PEPPER_EVENT_OBJECT_DESTROY, 0, SurfaceDestroyListener, this );

  mSurfaceCommitListener = pepper_object_add_event_listener( reinterpret_cast< pepper_object_t* >( surface ), PEPPER_EVENT_SURFACE_COMMIT, 0, SurfaceCommitListener, this );
...
void Shell::OnSurfaceDestroy( pepper_object_t *surface )
{
